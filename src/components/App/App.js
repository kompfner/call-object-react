...
export default function App() {
  ...
  const startJoiningCall = useCallback((url, asOwner) => {
    const newCallObject = DailyIframe.createCallObject({
      ...
      dailyConfig: {
        ...
        // -- v2 --
        // v2CamAndMic: false, // default: true
        // -- v1 --
   ...
   */
  useEffect(() => {
    window.DailyIframe = DailyIframe;
    ...
    callObject.on('track-started', (e) => {
      console.log('[EVENT] track started!', e);
    });

    callObject.on('track-stopped', (e) => {
      console.log('[EVENT] track stopped!', e);
    });

    callObject.on('cpu-load-change', (e) => {
      console.log(
        '[EVENT] cpu load changed!',
        e.cpuLoadState,
        e.cpuLoadStateReason
      );
    });

    callObject.on('face-counts-updated', (e) => {
      console.log('[EVENT] face counts updated!', e.faceCounts);
    });
    ...
    window.switchMicToExternal = async () => {
      const availableDevices = await callObject.enumerateDevices();
      const external = availableDevices.devices.find(
        (d) => d.kind === 'audioinput' && d.label.includes('External')
      );
      const newDevices = await callObject.setInputDevicesAsync({
        audioDeviceId: external.deviceId,
      });
      console.log('done switching. result: ', newDevices);
    };

    window.switchMicToMac = async () => {
      const availableDevices = await callObject.enumerateDevices();
      const mac = availableDevices.devices.find(
        (d) => d.kind === 'audioinput' && d.label.includes('MacBook')
      );
      const newDevices = await callObject.setInputDevicesAsync({
        audioDeviceId: mac.deviceId,
      });
      console.log('done switching. result: ', newDevices);
    };
    ...
    window.bgImgPreset = async (index) => {
      const newInputSettings = await callObject.updateInputSettings({
        video: {
          processor: { type: 'background-image', config: { source: index } },
        },
      });
      console.log('done updating input settings. result: ', newInputSettings);
    };
    ...
    window.bgImgMalformedURL = async () => {
      try {
        const newInputSettings = await callObject.updateInputSettings({
          video: {
            processor: {
    ...
    window.bgImgCustomArrayBuffer = async (useAlternate = false) => {
      try {
        const dataURL = useAlternate
    ...
    window.bgImgCustomBase64 = async (useAlternate = false) => {
      try {
        const dataURL = useAlternate
          ? 'data:image/png;base64,'
          : 'data:image/png;base64,';
    ...
    window.updateLocalParticipant = (updates) => {
      callObject.updateParticipant('local', updates);
    };

    window.updateRemoteParticipant = (updates) => {
      const firstRemoteParticipantId = Object.keys(
        callObject.participants()
      ).find((id) => id !== 'local');
      callObject.updateParticipant(firstRemoteParticipantId, updates);
    };

    window.updateTwoRemoteParticipants = (updates) => {
      const remoteParticipantIds = Object.keys(
        callObject.participants()
      ).filter((id) => id !== 'local');
      callObject.updateParticipants({
        [remoteParticipantIds[0]]: updates,
        [remoteParticipantIds[1]]: updates,
      });
    };

    window.knock = () => {
      callObject.requestAccess({ access: { level: 'full' }, name: 'paul' });
    };

    window.allowIn = () => {
      const firstWaitingParticipant = Object.keys(
        callObject.waitingParticipants()
      )[0];
      callObject.updateWaitingParticipant(firstWaitingParticipant, {
    ...
    window.setRemoteParticipantLayer = (layer) => {
      const firstRemoteParticipantId = Object.keys(
        callObject.participants()
      ).find((id) => id !== 'local');
      callObject.updateReceiveSettings({
        [firstRemoteParticipantId]: { video: { layer } },
      });
    };
    ...
    window.startCustomTrack = async () => {
      const stream = await navigator.mediaDevices.getUserMedia({
        video: { frameRate: 5, width: 100, height: 100 },
      });
      const track = stream.getVideoTracks()[0];
      callObject.startCustomTrack({ track, trackName: 'foo' });
    };

    window.showReceivedCustomTrack = (name = 'foo') => {
      const videoEl = document.getElementsByTagName('video')[0];
      const firstRemoteParticipantId = Object.keys(
        callObject.participants()
      ).find((id) => id !== 'local');
      const firstRemoteParticipant =
        callObject.participants()[firstRemoteParticipantId];
      const customTrack = firstRemoteParticipant.tracks[name].persistentTrack;
      videoEl.srcObject = new MediaStream([customTrack]);
    };

    window.startCustomAudioTrack = () => {
    ...
    window.subscribeToCustomAudioTrack = () => {
      const firstRemoteParticipantId = Object.keys(
        callObject.participants()
      ).find((id) => id !== 'local');
      callObject.on('track-started', (e) => {
  ...
  useEffect(() => {
    ...
    function handleTranscriptionMessage(event) {
      if (event) {
        logDailyEvent(event);
        console.log('[pk] transcription message!', {
